...
bool BP_RRTStar::plan()
{
  for (int i = 0; i < max_iters_; ++i)
  {
    ...
    {
      ROS_INFO("BP-RRT* connected to goal at iter %d, cost %.3f",
  ...
  ROS_WARN("BP-RRT* did not connect in %d iters", max_iters_);
  return false;
}
...
std::unique_ptr<Node> BP_RRTStar::sampleRandom() const
{
  ...
  Eigen::Vector3d p;
  p.x() = bounds_.low.x() +
          (bounds_.high.x()-bounds_.low.x())*(double(std::rand())/RAND_MAX);
  p.y() = bounds_.low.y() +
          (bounds_.high.y()-bounds_.low.y())*(double(std::rand())/RAND_MAX);
  p.z() = bounds_.low.z() +
          (bounds_.high.z()-bounds_.low.z())*(double(std::rand())/RAND_MAX);
